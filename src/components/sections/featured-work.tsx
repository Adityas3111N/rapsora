...
function ProjectCard({ project, index }: { project: Project; index: number }) {
    return (
        <motion.div
            ...
            className="group relative flex-shrink-0 w-[85vw] sm:w-[70vw] lg:w-[60vw] h-[550px] lg:h-[650px] overflow-hidden rounded-[2rem] lg:rounded-[3.5rem] bg-foreground/[0.03] dark:bg-white/[0.03] border border-foreground/5 dark:border-white/5 snap-center"
        >
            ...
            >
                <div className="absolute inset-0 bg-gradient-to-t from-background via-background/10 to-transparent z-10 transition-colors duration-500" />
                <div 
                    className="absolute inset-0 opacity-40 group-hover:opacity-60 transition-opacity duration-1000 grayscale-[40%] group-hover:grayscale-0"
                    style={{ 
            ...
            <div className="relative z-20 h-full w-full flex flex-col justify-end p-8 lg:p-12 text-left">
                ...
                <motion.div 
                    ...
                    className="absolute top-8 lg:top-12 right-8 lg:right-12 flex flex-col items-end"
                >
                    <div className="bg-background/40 backdrop-blur-2xl border border-foreground/10 rounded-2xl px-5 lg:px-6 py-3 lg:py-4 flex flex-col items-center shadow-2xl">
                        <span className="text-2xl lg:text-4xl font-black italic tracking-tighter" style={{ color: project.color }}>
                            {project.metric}
                        </span>
                        <span className="text-[9px] lg:text-[11px] font-black uppercase tracking-widest text-muted-foreground mt-1">
                            {project.metricLabel}
                        </span>
                    </div>
                </motion.div>
                ...
                <div className="max-w-md">
                    ...
                    >
                        ...
                        <span className="text-[10px] lg:text-xs font-bold uppercase tracking-[0.2em] text-muted-foreground">
                            {project.category}
                        </span>
                    </motion.div>
                    ...
                    <motion.h2 
                        ...
                        className="text-3xl lg:text-5xl font-black tracking-tight mb-3 text-foreground group-hover:text-primary transition-colors"
                    >
                        {project.title}
                    </motion.h2>
                    ...
                    <motion.p 
                        ...
                        className="text-base lg:text-lg text-muted-foreground mb-6 lg:mb-8 leading-relaxed line-clamp-2 lg:line-clamp-none"
                    >
                        {project.description}
                    </motion.p>
                    ...
                    <Link 
                        href={project.link}
                        className="group/btn relative inline-flex items-center gap-4 bg-foreground text-background px-6 lg:px-8 py-3.5 lg:py-4 rounded-full font-black text-xs lg:text-sm transition-all hover:scale-105 active:scale-95 shadow-[0_20px_40px_rgba(var(--foreground-rgb),0.1)]"
                    >
                        View Case Study
                        <div className="flex h-5 lg:h-6 w-5 lg:w-6 items-center justify-center rounded-full bg-background/20 group-hover/btn:rotate-45 transition-transform">
